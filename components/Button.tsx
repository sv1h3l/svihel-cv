...
const Button: FC<ButtonProps> = ({ iconSrc, label, url, triggerKey }) => {
  ...
  const handleAuxClick = (event: React.MouseEvent) => {
    if (event.button === 1) { // Kontrola, zda bylo kliknuto kolečkem myši
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (triggerKey && event.key === triggerKey) { // Kontrola, zda je triggerKey definovaný
  ...
  return (
    <button
      className="bg-white font-bold size-fit"
      onClick={handleClick}
      onAuxClick={handleAuxClick} // Přidání posluchače události pro kliknutí kolečkem myši
    >
      <div className="flex p-3 outline outline-1 outline-gray-600 rounded-lg gap-4 transition-all duration-75 ease-in-out hover:outline-2 hover:border-gray-800 hover:scale-110 hover:translate-x-3 box-border">
