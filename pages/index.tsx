...
export default function Index({
	...
	const [showResidue, setResidue] = useState(false);
	...
	useEffect(() => {
		...
		setTimeout(() => setResidue(true), 1300);
	}, []);

	return (
		<>
			<Head>
				<title>Jakub Švihel</title>
				<meta
					name="description"
					content="Portfolio Jakuba Švihla – full-stack vývojář, zkušenosti v komerční i akademické sféře, projekty, dovednosti a certifikáty."
				/>
				<meta
					name="keywords"
					content="Jakub Švihel, full-stack developer, portfolio, React, Next.js, Tailwind, TypeScript, projekty, certifikáty, dovednosti, vzdělání"
				/>
				<meta
					name="author"
					content="Jakub Švihel"
				/>
				...
				<meta
					property="og:url"
					content="https://svihel-cv.web.app/"
				/>
				<meta
					property="og:image"
			...
			<main className="bg-[#3C3D37] px-1 lg:px-2 font-roboto ">
				<div className="flex w-full flex-col pb-1 lg:pb-2 text-gray-200 items-center">
					<div
						className={`
                                bg-[#191919] border-t-2 relative border-[#b7a71d] rounded-b-3xl px-3 pt-1 pb-1 sm:px-6 sm:py-5 mb-4 lg:mb-6
					...
					<JTab
						...
						{
							<div className="relative grid">
								<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-100" : "opacity-0 absolute"}`}>
									<JGrid gridArray={personalPageGeneralDataCZ.employment} />
								</div>
								<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-0 absolute" : "opacity-100"}`}>
									<JGrid gridArray={personalPageGeneralDataEN.employment} />
								</div>
							</div>
						}
					</JTab>
					<JTab
						...
						<div className="relative grid">
							<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-100" : "opacity-0 absolute"}`}>
								<JGrid gridArray={personalPageGeneralDataCZ.skills} />
							</div>
							<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-0 absolute" : "opacity-100"}`}>
								<JGrid gridArray={personalPageGeneralDataEN.skills} />
							</div>
						</div>
					</JTab>
					<JTab
						...
						<div className="relative grid">
							<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-100" : "opacity-0 absolute"}`}>
								<JGrid gridArray={personalPageGeneralDataCZ.education} />
							</div>
							<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-0 absolute" : "opacity-100"}`}>
								<JGrid gridArray={personalPageGeneralDataEN.education} />
							</div>
						</div>
					</JTab>
					<JTab
						...
						<div className="relative grid">
							<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-100" : "opacity-0 absolute"}`}>
								<JGrid gridArray={personalPageGeneralDataCZ.certificates} />
							</div>
							<div className={`transition-all duration-500 col-start-1 row-start-1 ${isCzech ? "opacity-0 absolute" : "opacity-100"}`}>
								<JGrid gridArray={personalPageGeneralDataEN.certificates} />
							</div>
						</div>
					</JTab>
					<Image
						className={`pt-4 px-16 lg:pt-5 -mb-1
                                transition-all duration-700
                                ${showResidue ? "opacity-100" : "opacity-0"}
                            `}
						src="/images/line.webp"
						width={750}
						height={0}
